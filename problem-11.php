...
array( 78,17,53,28,22,75,31,67,15,94,03,80,4,62,16,14,9,53,56,92),
array( 16,39,5,42,96,35,31,47,55,58,88,24,00,17,54,24,36,29,85,57),
...
array( 4,52,8,83,97,35,99,16,07,97,57,32,16,26,26,79,33,27,98,66),
...
array( 20,73,35,29,78,31,90,1,74,31,49,71,48,86,81,16,23,57,05,54),
...
for($i=0;$i<count($num)-3;$i++){
    for($j=0;$j<count($num[$i])-3;$j++){
        $baris = $num[$i][$j] * $num[$i+1][$j] * $num[$i+2][$j] * $num[$i+3][$j];
        $kolom = $num[$i][$j] * $num[$i][$j+1] * $num[$i][$j+2] * $num[$i][$j+3];
        $diag = $num[$i][$j] * $num[$i+1][$j+1] * $num[$i+2][$j+2] * $num[$i+3][$j+3];
        if($baris>$prod){
            $prod = $baris;
            $b1 = $i;
            $k1 = $j;
